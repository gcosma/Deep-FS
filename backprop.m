...
w_class = 0.1*randn(numpens,Odim); 
topbiases = 0.1*randn(1,Odim);
...
%%%% Error rates 
   ...
   for batch = 1:testnumbatches
     ...
     targetout = targetout./repmat(sum(targetout,2),1,Odim);
     [I J]=max(targetout,[],2); 
     [I1 J1]=max(target,[],2); 
...
%     end  
  ...
  for batch = 1:numbatches/numcases
    %fprintf(1,'epoch %d batch %d\r',epoch,batch);
    data = zeros(numcases^2,numdims); 
    temp_h2 = zeros(numcases^2,numpens); 
    targets = zeros(numcases^2,size(batchtargets,2)); 
    tt1=(batch-1)*numcases+1:batch*numcases; 
    for tt=1:numcases
      data( (tt-1)*numcases+1:tt*numcases,:) = batchdata(:,:,rr(tt1(tt))); 
      temp_h2( (tt-1)*numcases+1:tt*numcases,:) = temp_h2_train(:,:,rr(tt1(tt))); 
      targets( (tt-1)*numcases+1:tt*numcases,:) = batchtargets(:,:,rr(tt1(tt))); 
    end  
...
err_III=[test_err(epoch), train_err(epoch)];
