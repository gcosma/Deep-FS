...
% Do decomversion.
 ...
 w_class = reshape(X(xxx+1:xxx+numpens*size(target,2)),numpens,size(target,2));
 xxx = xxx+numpens*size(target,2);
 hidbiases = reshape(X(xxx+1:xxx+numhids),1,numhids);
 xxx = xxx+numhids;
 penbiases = reshape(X(xxx+1:xxx+numpens),1,numpens);
 xxx = xxx+numpens;
 topbiases = reshape(X(xxx+1:xxx+size(target,2)),1,size(target,2));

 xxx = xxx+size(target,2);
